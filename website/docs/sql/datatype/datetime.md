...
these data types are described in [???](#functions-datetime). Dates are
...
[date_style](#date_style) setting. Set it to `MDY` to select
...
[table_title](#datatype-datetime-date-table) shows some possible inputs
for the `date` type.
...
information. Input for this type is illustrated in
[table_title](#datatype-datetime-time-table).
...
The output format of the date/time types follows the ISO 8601 format,
...
The formatting function `to_char` (see [???](#functions-formatting)) is
...
Table [table_title](#datatype-timezone-table) shows examples of time
zone input.
...
looks like this: P \<quantity\> \<unit\> \[\<quantity\> \<unit\> \...\]
\[T \[\<quantity\> \<unit\> \...\]\] The string must start with a `P`,
and may include a `T` that introduces the time-of-day units. The
available unit abbreviations are given in
[table_title](#datatype-interval-iso8601-units). Units may be omitted,
...
In the alternative format: P \[\<years\>-\<months\>-\<days\>\] \[T
\<hours\>:\<minutes\>:\<seconds\>\] the string must begin with `P`, and
...
[table_title](#datatype-interval-input-examples) shows some examples of
...
[???](#functions-datetime)) are available for adjusting days and hours
...
`sql_standard` or `iso_8601`. using the command `SET intervalstyle`. The
default is the `iso_8601` format.
[table_title](#interval-style-output-table) shows examples of each
output style.
