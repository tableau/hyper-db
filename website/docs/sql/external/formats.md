...
which the external format will be processed. Some external formats carry
schema information (e.g., Apache Parquet) while others do not (e.g.,
CSV). While Hyper can infer the schema of formats possessing schema
information automatically, the schema has to be given explicitly for
formats without schema information (or is taken from the target table in
a COPY statement); see [???](#external-option-columns).

The format of an external source is set through the
[FORMAT](#external-option-format) option. If the option is not
specified, Hyper will try to infer the format given the file extension.
If multiple files are read, all have to possess the same extension for
the inferral to succeed.

The following formats are supported:

  Format                                       `format` Option Value   Recognized File Extensions                     Schema Inference?   Description                                                                                                           Supported Options
  -------------------------------------------- ----------------------- ---------------------------------------------- ------------------- --------------------------------------------------------------------------------------------------------------------- ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  [Text](#external-format-text)                `'text'`                `.csv`, `.csv.gz`                              No                  Text format; as in PostgreSQL. Optionally, gzip compressed.                                                           [SANITIZE](#external-option-sanitize), [COMPRESSION](#external-option-compression), [???](#external-option-columns), [DELIMITER](#external-option-delimiter), [NULL](#external-option-null), [ENCODING](#external-option-encoding), [ON_CAST_FAILURE](#external-option-on-cast-failure)
  [CSV](#external-format-csv)                  `'csv'`                                                                No                  Comma Separated Value format; as in PostgreSQL. Optionally, gzip compressed.                                          [SANITIZE](#external-option-sanitize), [COMPRESSION](#external-option-compression), [???](#external-option-columns), [DELIMITER](#external-option-delimiter), [NULL](#external-option-null), [ENCODING](#external-option-encoding), [ON_CAST_FAILURE](#external-option-on-cast-failure), [HEADER](#external-option-header), [QUOTE](#external-option-quote), [ESCAPE](#external-option-escape), [FORCE_NOT_NULL](#external-option-force-not-null), [FORCE_NULL](#external-option-force-null)
  [Apache Parquet](#external-format-parquet)   `'parquet'`             `.parquet`                                     Yes                 The [Apache Parquet format](https://parquet.apache.org/); both version 1 and 2 supported                              [SANITIZE](#external-option-sanitize), [COMPRESSION](#external-option-compression), [???](#external-option-columns)
  [Apache Iceberg](#external-format-iceberg)   `'iceberg'`             Specified path must point to table directory   Yes                 The [Apache Iceberg format](https://iceberg.apache.org/); version 1 and 2 are supported; version 3 is not supported   [SANITIZE](#external-option-sanitize)
...
`option => value`, and separated by commas. In the [???](#sql-copy) and
[???](#sql-createexternaltable) statement, options are specified in the
`WITH` clause, in the [`external`](#functions-srf-external) set
returning function, options are specified as function arguments after
the initial argument describing the source. For example, the following
statements all read from a CSV file `products.csv`, using field
delimiter `'|'`:

    COPY products FROM 'products.csv' WITH (FORMAT => 'csv', DELIMITER => '|')
    CREATE TEMPORARY EXTERNAL TABLE products (name text, price int) FOR 'products.csv' WITH (FORMAT => 'csv', DELIMITER => '|')
    SELECT * FROM external('products.csv', FORMAT => 'csv', DELIMITER => '|', COLUMNS => DESCRIPTOR(name text, price int))

Options available for each format, respectively, are described in detail
in the section of the respective format below. The following options are
available for all or multiple external formats:

`FORMAT => 'format_name'`

:   The format, as specified in [table_title](#table-external-formats).
...
:   If set to `'auto'` (default) then any input file having an extension
    ...
    Not available for Apache Parquet, as this format handles its own
...
:   The schema of the file. Only necessary when using the `external`
    function to read the data, as `COPY` and
    `CREATE TEMPORARY EXTERNAL TABLE` will assume the file to have the
    same schema as the table. See [COLUMNS
    option](#external-option-columns) for details.
...
null string is used in place of columns that are null. An error will be
raised if any line of the input file contains more or fewer columns than
are expected.
...
:   Specifies the behavior when a cast failure occurs, that is, when the
    ...
    then an error is raised. The default is `error`. Note that you can
    use the [???](#external-option-columns) option to explicitly define
    columns as nullable when reading with the `external` function.
...
subdirectories. The Iceberg table can be specified as described in the
[???](#functions-srf-external), however the `ARRAY` syntax is not
